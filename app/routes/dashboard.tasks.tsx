...
const statuses = {
  activa: 'text-gray-500 bg-gray-100/10',
  terminada: 'text-green-400 bg-green-400/10',
          ...
          <li key={deployment.id} className="relative flex items-center space-x-4 px-4 py-4 sm:px-6 lg:px-8">
            <div className="min-w-0 flex-auto">
              <div className="flex items-center gap-x-3">
                ...
                <h2 className="min-w-0 text-sm/6 font-semibold text-white">
                  <a href={deployment.href} className="flex gap-x-2">
                    <span className="whitespace-nowrap">{deployment.taskName}</span>
              ...
              <div className="mt-3 flex items-center gap-x-2.5 text-xs/5 text-gray-400">
                ...
                <p className="whitespace-nowrap">{deployment.expirationDate}</p>
              </div>
            </div>
            {deployment.assignedUsers.length > 0 ? (
              deployment.assignedUsers.map((user) => (
                <span  title={user} key={user} href="#" className="flex-none">
                  <img
                    key={user}
                    src={`https://ui-avatars.com/api/?name=${user}&size=32&background=random`}
                    alt={user}
